...
class FH {
  ...
  /**
   * Cleans user input with htmlspecialchars
   * @method sanitize
   ...
   */
  public static function displayErrors($errors, $admin=false) {
    $hasErrors = (!empty($errors))? ' has-errors' : '';
    $background = ($admin == false) ? 'bg-light' : 'bg-secondary';
    $text = ($admin == false) ? 'text-danger' : 'text-light';
    $html = '<div class="form-errors"><ul class="'.$background.$hasErrors.'">';
    foreach($errors as $field => $error) {
      $html .= '<li class="'.$text.'">'.$error.'</li>';
    }
    $html .= '</ul></div>';
    return $html;
  }
